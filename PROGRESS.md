...
> 最后更新：R40（K06 重构为行卡片入口结构 + 新增 K19/K20 二级源选择页）
...
| 添加内容 K06 | ✅ 完成（R40 重构）| 粘贴链接行 + 最近横滑 + 自动同步/手动添加入口行卡片，二级页面 K19/K20 展开 6+4 来源 |
...
5 个区块严格分层，所有"行卡片"统一规格（白底、1px #EEEEEE、12px 圆角、padding 12px、左右 margin 16px、36×36 图标方块 10px 圆角）：

1. **TopHeader**：左"取消" + 中"添加内容" + 右 28px 占位
2. **粘贴链接行**：浅橙底 Link 图标，输入合法 URL 后图标变绿 ✓，右侧出现"保存"小按钮
3. **最近浏览/下载横滑**：6 张 130×84 卡，28×28 浅橙 emoji 方块，多选 + 底部"+ 添加到「XXX」"浮条
4. **自动同步入口行卡**：橙 RefreshCw 图标 + 描述"浏览器 · 微信 · 网盘 · 截图 · 邮件 · 下载" → `/knowledge/:kbId/add-from-auto`
5. **手动添加入口行卡**：灰 Hand 图标 + 描述"上传文件 · 第三方分享 · AI 对话 · 扫一扫" → `/knowledge/:kbId/add-from-manual`

「我的速记」额外显示底部「新建速记」行卡片。

#### 二级页 K19 自动同步源选择（6 行）
浏览器历史 / 微信收藏 / 网盘 / 邮件附件 / 系统截图 / 下载文件，每行 emoji + "XX · 已连接"，点击进入对应 K14 列表（cloud 直接跳 `/knowledge/add-from/cloud-files`，去掉了原网盘 picker）。

#### 二级页 K20 手动添加源选择（4 行）
上传文件 / 第三方 App 分享 / AI 对话保存 / 扫一扫，点击进入 K15/K16/K17/K18。
...
| K06 | 添加内容 BottomSheet | 粘贴链接行 + 最近横滑 + 自动同步/手动添加入口行卡 |
| K19 | 自动同步源选择 | 6 行：浏览器历史/微信收藏/网盘/邮件附件/系统截图/下载文件 |
| K20 | 手动添加源选择 | 4 行：上传文件/第三方分享/AI 对话/扫一扫 |
