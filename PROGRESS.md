...
> 最后更新：R45（K06 删顶部粘贴卡片 + output 阶段卡片样式统一）
...
- **R45**：K06 顶部「粘贴链接快速添加」输入卡片彻底删除，K06 内容只保留三块：最近浏览/下载 → 自动同步 → 手动添加（粘贴功能现在只在「手动添加 → 第三方」子页存在）。K05 output 模式四阶段卡片样式统一：所有卡片背景白底（去掉「提炼中」的浅橙底）；AI 提炼的视觉强调只放在 active tone 卡片左上角的 sparkle 图标盒（`h-11 w-11` 浅橙 `#FFF1E6` 底 + 橙图标 20px），其它阶段图标盒保持 32×32 灰底；正文字号下调到 13/11 px 让 116px 固定高度更松。
- **R44**：K06 添加内容由 K05 内部 BottomSheet 改为独立路由页 `/knowledge/upload`，BottomSheet 视觉不变（永远 open，关闭即 `navigate(-1)`）。原因：从 K06 跳子页（K14-K18）后浏览器返回会失去 K05 的 `showUpload` 状态，造成「点返回直接退到 K05」的体验断裂。改为路由后，子页 `navigate(-1)` 自然回到 K06。`K05_KnowledgeDetail` 移除 `showUpload` state，三个入口（empty state `+`、悬浮 `+`、更多菜单"添加内容"）改为 `navigate('/knowledge/upload')`。K06 内 `取消` 按钮接 `navigate(-1)`、点击区扩大；移除原 `navigateTo(route)` 的 close+setTimeout 套路。K16 第三方页重做为「粘贴链接添加」：删除原圆形分享图标 + 主标题 + 段落说明，主体只保留一张垂直水平居中的浅橙粘贴卡片（44×44 白底 Link2 + 「粘贴链接」+ 副标题「小红书、知乎、B 站等都支持」 + 28×28 白底圆形 ArrowUpRight），整张可点 → mock 已添加链接 toast 并 `navigate(-1)`
