...
export default function SaveToKnowledgeBaseSheet({ open, onClose, sourceContent, onSaved, successToast, title = '添加到知识库' }: SaveSheetProps) {
    ...
    <>
      ...
      >
        <div className="flex h-full min-h-0 flex-col px-5 pt-2">
          <p className="text-caption text-ink-secondary mb-4 text-center">选择要保存的知识库</p>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto scrollbar-hide pb-3">
            ...
            {!hasMatches && (
              ...
              >
                没有找到相关知识库，可以新建一个 <span className="text-brand-orange">→</span>
              </button>
            )}
          </div>
