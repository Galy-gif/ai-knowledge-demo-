...
const PARA_FOLDER_FILES: Record<string, FolderMockFile[]> = {
  ...
  folder_plugins: [
    ...
    { id: 'fpl_7', title: '端侧资料包插件 PoC', type: 'url', meta: '网页 · GitHub', time: '1 个月前' },
  ],
    ...
    items: [
      ...
      },
      {
        ...
        file: makeWorkflowFile(
          ...
          '## 团队脑暴笔记\n\n照片中记录了三个方向：移动端搜索入口、资料包自动沉淀、从网页生成小应用。当前最适合进入复盘报告的是“资料离成果还差几步”的工作流表达。',
          '5张照片',
        ),
      },
    ],
    ...
    items: [
      ...
      },
      {
        ...
        file: makeWorkflowFile(
          ...
          '## 数据趋势摘要\n\nQ3 的关键变化是“搜索后保存”的比例显著上升。说明用户不再满足于即时答案，而希望把有价值的网页和结论沉淀到长期资料包里。',
        ),
      },
    ],
  },
...
const IDEA_CARDS: IdeaCardData[] = [
  ...
  },
  {
    ...
    label: '用户洞察',
    time: '1 月前',
    body: "用户调研：把'保存'按钮换成'放进资料包'后，使用率提升 3 倍。命名重要。",
    related: ['产品方法论'],
  },
]
  ...
  return (
    <div className="absolute inset-0 z-[55] flex items-center justify-center bg-black/35 px-8">
      <div className="w-full rounded-[16px] bg-white p-5 text-center shadow-sheet">
        ...
        <p className="text-[15px] font-semibold leading-5 text-ink-primary">AI 正在重新整理你的资料包...</p>
      ...
      <div className="-translate-y-10 flex flex-col items-center text-center">
        ...
        <p className="mt-2 text-[13px] leading-5 text-[#4A4A4A]">
          {!hasMode
            ...
            : isFree
              ? '想到什么就丢进来'
              : `这个资料包会用「${getManagementModeLabel(mode)}」帮你整理，开始添加吧`}
        </p>
    ...
    <div className="flex flex-col h-full min-h-0 overflow-hidden relative bg-surface-card">
      ...
      <div className="flex-shrink-0 flex items-center h-14 px-4 bg-white border-b border-line-base">
        {isSelecting ? (
          ...
          <>
            ...
            <span className="absolute left-16 right-16 text-center text-h2 text-ink-primary truncate">{activeBase?.name ?? '资料包'}</span>
            <button
              type="button"
      ...
      {!isSelecting && <div className="relative z-[var(--z-bottom-bar)] flex-shrink-0 border-t border-line-base bg-white px-4 py-3 shadow-[0_-8px_18px_rgba(15,23,42,0.04)]">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="flex-1 min-w-0 truncate text-body text-ink-placeholder">问 AI 关于「{activeBase?.name ?? '资料包'}」…</span>
      ...
      >
        <div className="px-5 pt-2 pb-4">
          <p className="text-caption text-ink-secondary text-center mb-4">选择目标资料包</p>
          <div className="space-y-2">
