...
import { Send, BookmarkPlus, ChevronDown, Copy, Share2, ThumbsUp, ThumbsDown } from 'lucide-react'
...
const ROUND1_SUMMARY = '运营资料包的核心在于建立可持续的内容生命周期管理体系，涵盖运营目标、内容分发、社区运营、数据看板 4 个结构化模块。'

const ROUND2_ANSWER = `衡量资料包运营效果可以从以下维度建立评估体系：

**核心指标**
- **内容使用率**：每篇文档的月均阅读次数，衡量内容价值
- **检索满意率**：用户首次搜索即找到目标内容的比例
- **知识转化率**：阅读后产生实际行动（收藏、分享、引用）的比例

**运营健康度指标**
- 内容更新频率（建议每周不低于 3 篇）
- 过期内容占比（超过6个月未更新视为需审查）
- 用户贡献率（团队成员主动提交内容的比例）

**工具推荐**：可结合内部 OKR 系统与本工具的数据看板进行追踪。`
...
function Round2Content({ text }: { text: string }) {
  const { hlTexts, hlColors } = useDocumentReader()
  return <>{renderMarkdown(text, hlTexts, hlColors)}</>
}

function Round1SummaryText() {
  const { hlTexts, hlColors } = useDocumentReader()
  return (
    <p className="text-body text-ink-secondary leading-relaxed">
      <HighlightedText text={ROUND1_SUMMARY} highlights={hlTexts} colorMap={hlColors} />
    </p>
  )
}

export default function Q05_AiAnswerMulti() {
  const { state } = useLocation()
  const navigate = useNavigate()

  const { showToast } = useUser()
  const { question = '资料包如何运营？', followUp = '如何衡量运营效果？' } = (state as { question?: string; followUp?: string }) ?? {}
  ...
  const handleSend = () => {
    ...
    navigate('/ask/answer-multi', { state: { question: followUp, followUp: input, selectedKbIds, selectedKbNames } })
  }
  ...
  return (
    <div className="flex flex-col h-full relative bg-white">
      <TopHeader title="AI 问答" showBack />

      <DocumentReader
        docId={`ai:${question.slice(0, 50)}`}
        docType="ai-answer"
        bottomPx={72}
      ...
      >
        ...
        <div className="bg-surface-card rounded-card overflow-hidden border border-line-base">
          <button
            onClick={() => setRound1Expanded(v => !v)}
            className="w-full flex items-center justify-between p-4"
          >
            <p className="text-body font-medium text-ink-primary flex-1 truncate text-left">{question}</p>
            <ChevronDown
              size={16}
              className={`text-ink-placeholder flex-shrink-0 ml-2 transition-transform ${round1Expanded ? 'rotate-180' : ''}`}
            />
          </button>
          {!round1Expanded ? (
            <div className="px-4 pb-3 border-t border-line-base/50">
              <p className="text-caption text-ink-placeholder mt-2 line-clamp-2 leading-relaxed">
                {ROUND1_SUMMARY}
              </p>
              <div className="flex gap-1.5 mt-2 flex-wrap">
                {['运营目标', '内容分发', '社区运营', '数据看板', '结论'].map(tag => (
                  <span key={tag} className="text-micro px-2 py-0.5 bg-white rounded-pill border border-line-base text-ink-placeholder">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="px-4 pb-4 border-t border-line-base/50 pt-3">
              <Round1SummaryText />
            </div>
          )}
        </div>
        ...
        <div className="flex justify-end">
          <div className="bg-brand-orange/10 rounded-card rounded-tr-sm px-4 py-3 max-w-[85%]">
            <p className="text-body text-ink-primary">{followUp}</p>
          </div>
        </div>

        {/* Round 2 answer (streaming) */}
        <div>
          ...
          <StreamingText
            ...
            render={(displayed, streaming) => (
              <div>
                {streaming && (
                  <div className="w-1.5 h-4 bg-brand-orange rounded-full animate-pulse mb-2" />
                )}
                <div><Round2Content text={displayed} /></div>
                {!streaming && (
                  <div className="flex gap-2 mt-4 flex-wrap">
                    ...
                    <button
                      onClick={() => { navigator.clipboard.writeText(ROUND2_ANSWER).catch(() => {}); showToast('已复制') }}
                      className="flex items-center gap-1.5 px-3 py-2 bg-surface-card rounded-pill text-caption text-ink-secondary"
                    >
                      <Copy size={14} />复制
                    </button>
                    <button
                      onClick={() => openKbSheet('AI 追问整理', ROUND2_ANSWER)}
                      className="flex items-center gap-1.5 px-3 py-2 bg-surface-card rounded-pill text-caption text-ink-secondary"
                    >
